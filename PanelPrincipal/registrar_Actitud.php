...
<h2>Buscar Alumno - Actitud</h2>
  ...
  <div class="buscarAlumnoExact">
        ...
        <button class="botonAlumnoExact" onclick="buscarAlumnoAct()">Buscar</button>
        </div>
  <form style="display: flex;" method="get">
    <div class="form-group" style="width: 20%;">
      <select name="seccionAct" id="seccionAct">
    ...
    <button class="botonSeccionAct"  name="submit" onclick="SeccionAct()" id="buscarSeccionAct" style="margin-left: 29%;" >Buscar</button>
  </form>
 

  <div id="formularioContainerAct"></div>




<?php
if (isset($_GET['grados_A_Act'])) {
    

    $gradoSeleccionado = $_GET['grados_A_Act'];
    ...
    if ($result !== false && $result->num_rows > 0) {
        ...
        while ($fila = $result->fetch_assoc()) {
            $codigoAct = $fila["idAlumno"];
            $nombreAct = $fila["nomAlumno"];
            $apellidoAct = $fila["apeAlumno"];
            $dniAct = $fila["dniAlumno"];

            echo '<tr>
                    <td>' . $nombreAct. '  ' . $apellidoAct . '</td>
                   
                    <td>' . $dniAct . '</td>
                    <td>
                      <button type="button" name="RegistarIncidencia" data-id="'. $codigoAct .'"data-dni="' . $dniAct . '" data-nombre="' . $nombreAct . '">Registrar</button>
                    </td>
                    <td>
                    <button type="button" name="mostrarIncidencia" data-id="'. $codigoAct .'"data-dni="' . $dniAct . '" data-nombre="' . $nombreAct . '">Mostrar</button>
                  </td>
...
if (isset($_GET['grados_B_Act'])) {
    

  $gradoSeleccionado = $_GET['grados_B_Act'];
...
if (isset($_GET['grados_C_Act'])) {
    

  $gradoSeleccionado = $_GET['grados_C_Act'];
...
if (isset($_GET['grados_D_Act'])) {
    

  $gradoSeleccionado = $_GET['grados_D_Act'];
...
if (isset($_GET['grados_E_Act'])) {
    

  $gradoSeleccionado = $_GET['grados_E_Act'];
...
// Empezamos desde 1 para omitir la fila de encabezado
  ...
  $variableSeccion = document.getElementById("buscarSeccionAct");

function SeccionAct() {

  var seccion = document.getElementById('seccionAct').value;
  console.log(seccion);
  event.preventDefault();
  if (seccion == 'A') {
    $variableSeccion.style.display = 'none';
    var formularioGradosHTML = '<div id="formularioGradosAct" class="hidden">' +
      '<form style="display: flex;" method="get">' +
      '<div class="form-group" style="width: 20%;">' +
      '<select name="grados_A_Act" id="gradosAct">' +
      ...
      '</form>' +
      '</div>';
    var formularioContainer = document.getElementById('formularioContainerAct');
  ...
  if (seccion == 'B') {
    ...
    var formularioGradosHTML = '<div id="formularioGradosAct" class="hidden">' +
      '<form style="display: flex;" method="get">' +
      '<div class="form-group" style="width: 20%;">' +
      '<select name="grados_B_Act" id="gradosAct">' +
      '<option value="1ER">1ER</option>' +
      '<option value="2DO">2DO</option>' +
      '<option value="3RO">3RO</option>' +
      '<option value="4TO">4TO</option>' +
  ...
  if (seccion == 'C') {
    ...
    var formularioGradosHTML = '<div id="formularioGradosAct" class="hidden">' +
      '<form style="display: flex;" method="get">' +
      '<div class="form-group" style="width: 20%;">' +
      '<select name="grados_C_Act" id="gradosAct">' +
      '<option value="1ER">1ER</option>' +
      '<option value="2DO">2DO</option>' +
      '<option value="3RO">3RO</option>' +
      '<option value="4TO">4TO</option>' +
  ...
  if (seccion == 'D') {
    ...
    var formularioGradosHTML = '<div id="formularioGradosAct" class="hidden">' +
      '<form style="display: flex;" method="get">' +
      '<div class="form-group" style="width: 20%;">' +
      '<select name="grados_D_Act" id="gradosAct">' +
      '<option value="1ER">1ER</option>' +
      '<option value="2DO">2DO</option>' +
      '<option value="3RO">3RO</option>' +
  ...
  if (seccion == 'E') {
    ...
    var formularioGradosHTML = '<div id="formularioGradosAct" class="hidden">' +
      '<form style="display: flex;" method="get">' +
      '<div class="form-group" style="width: 20%;">' +
      '<select name="grados_E_Act" id="gradosAct">' +
      '<option value="1ER">1ER</option>' +
      '<option value="2DO">2DO</option>' +
...
<script>
  ...
  function redirigirConParametros(dni, nombre,id) {
    ...
    window.location.href = 'ingresarDatosActitud.php?dni=' + dni + '&nombre=' + nombre + '&id='+ id;
  }

  // Capturar el evento de clic en los botones
  document.addEventListener('DOMContentLoaded', function() {
    var botonesIncidencia = document.getElementsByName('RegistarIncidencia');
...
//utilizar tu archivo php
window.location.href = 'mostrar_Incidencias.php?dni=' + dni + '&nombre=' + nombre + '&id='+ id;
}


  document.addEventListener('DOMContentLoaded', function() {
    var botonesIncidencia = document.getElementsByName('mostrarIncidencia');
    // var botonesJustificacion = document.getElementsByName('justificacion');
